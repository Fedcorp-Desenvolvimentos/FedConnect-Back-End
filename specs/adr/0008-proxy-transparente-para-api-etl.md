...
5. **Autorização**: JWT + `IsAuthenticated` + nível em `NIVEIS_TELA` (hoje `("admin",)`, PA-014); 403 no formato `{"erro": "sem_acesso", "mensagem": ...}`. O FedHub não conhece o usuário final: o e-mail do operador vai no header `X-Operador` para o log do FedHub (trilha de leitura de funcionário, decisão 0043 do FedHub).
