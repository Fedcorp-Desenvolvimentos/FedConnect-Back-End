...
- **Complemento (2026-09-23):** o acesso deixou de ser "para todos": as rotas de indicadores e o cadastro de metas passaram a exigir nível `admin` ou `ti` (`users.permissions.IsAdminOrTi`), decisão espelhada na questão de mesmo teor do registro do frontend. As specs que citam essa revisão apontam para esta questão.
...
- **Status:** fechada (2026-09-24) · **Dono:** Hamilton (gestor comercial), via Lucas Guidi · **Severidade:** média
- **Trava:** RF-IEX-003 (janela do período `mes` e do anterior equivalente).
- **Questão:** o protótipo e a v1 contavam "este mês" do dia 1 até a data de referência (mês corrente até hoje) e comparavam com o mesmo trecho do mês anterior. O dono relatou que o filtro estava errado.
