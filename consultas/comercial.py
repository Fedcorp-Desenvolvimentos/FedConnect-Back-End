    ...
    def post(self, request, *args, **kwargs):
        ...
        for cnpj_raw in cnpjs_to_process:
            ...
            try:
                ...
                cpfs_for_consult = []
                if cnpj_relacionamentos_result and 'Result' in cnpj_relacionamentos_result and cnpj_relacionamentos_result['Result']:
                    data_entry = cnpj_relacionamentos_result['Result'][0]
                    if 'Relationships' in data_entry and isinstance(data_entry['Relationships'], dict):
                        all_relationships = data_entry['Relationships'].get('CurrentRelationships', [])
                        
                        for relationship in all_relationships:
                            relationship_type = relationship.get('RelationshipType')
                            entity_tax_id_type = relationship.get('RelatedEntityTaxIdType')
                            entity_tax_id_number = relationship.get('RelatedEntityTaxIdNumber')

                            display_relationship_type = relationship_type.capitalize() if relationship_type else 'N/A'
                            
                            if relationship_type and relationship_type.upper() in ["QSA", "OWNERSHIP", "REPRESENTANTELEGAL"] \
                               and entity_tax_id_type and entity_tax_id_type.upper() == "CPF" \
                               and entity_tax_id_number:
                                ...
                                if len(cleaned_cpf) == 11:
                                    cpfs_for_consult.append({
                                        'cpf': cleaned_cpf,
                                        'related_entity_name': relationship.get('RelatedEntityName', 'N/A'),
                                        'relationship_name': relationship.get('RelationshipName', 'N/A'),
                ...
                for cpf_info in cpfs_for_consult:
                    ...
                    try:
                        ...
                        if cpf_result and 'Result' in cpf_result and cpf_result['Result']:
                            cpf_data_entry = cpf_result['Result'][0]
                            registration_data = cpf_data_entry.get('RegistrationData', {})
                            ...
                            if birth_date_str != 'N/A':
                                try:
                                    cpf_detailed_birthdate = pd.to_datetime(birth_date_str).strftime('%d/%m/%Y')
                                except ValueError:
                            ...
                            if 'Primary' in emails_data:
                                cpf_email_primary = emails_data['Primary'].get('EmailAddress', 'N/A')
                            if 'Secondary' in emails_data:
                                cpf_email_secondary = emails_data['Secondary'].get('EmailAddress', 'N/A')
                            ...
                            if 'Primary' in phones_data:
                                ...
                                if cpf_phone_primary == "+ ()": cpf_phone_primary = "N/A"
                            if 'Secondary' in phones_data:
                                ...
                                if cpf_phone_secondary == "+ ()": cpf_phone_secondary = "N/A"
                            ...
                            if 'Primary' in addresses_data:
                                ...
                                cpf_address_primary = re.sub(r'\s+', ' ', cpf_address_primary)
                                cpf_address_primary = re.sub(r'(\s*,\s*){2,}', ', ', cpf_address_primary)
                                cpf_address_primary = re.sub(r'(\s*-\s*){2,}', ' - ', cpf_address_primary)
                                if cpf_address_primary.strip() == ", - , / -": cpf_address_primary = "N/A"
                            if 'Secondary' in addresses_data:
                                ...
                                cpf_address_secondary = re.sub(r'\s+', ' ', cpf_address_secondary)
                                cpf_address_secondary = re.sub(r'(\s*,\s*){2,}', ', ', cpf_address_secondary)
                                cpf_address_secondary = re.sub(r'(\s*-\s*){2,}', ' - ', cpf_address_secondary)
                                if cpf_address_secondary.strip() == ", - , / -": cpf_address_secondary = "N/A"
                    ...
                    all_cpf_data.append({
                        ...
                        "CPF": cpf,
                        "Nome Relacionado ao CNPJ": cpf_info.get('related_entity_name', 'N/A'),
                        "Tipo de Relacionamento (CNPJ)": cpf_info.get('relationship_type', 'N/A'),
                        "Nome do Relacionamento (CNPJ)": cpf_info.get('relationship_name', 'N/A'),
        ...
        response['Content-Disposition'] = 'attachment; filename="resultados_consulta_massa_cpf.xlsx"'
        return response